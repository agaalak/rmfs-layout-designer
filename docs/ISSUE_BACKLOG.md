...
| SIM-P1-008 | Queue-lane runtime did not match requested pre-point behavior | P1 | fixed | Inspect station approach behavior after semantic correction | Queue is a station pre-point resource that can apply to one or all stations | Runtime still had queue-lane source-of-truth concepts | `queuePointLifecycle.ts`, `simulationEngine.ts`, `layoutSemantics.ts` | Added `QueuePoint`, migrated legacy queues, and routed loaded station visits through selected queue pre-points | `queuePointLifecycle.test.ts`, full E2E |
| SIM-P0-009 | Demo layout config overwrote user task-count changes and made only one robot run | P0 | fixed | Set task count above 1, initialize Small Demo, generate tasks | User simulation settings survive initialization and the default demo creates enough work for multiple robots | Layout-level `simulationConfig.taskCount=1` overwrote active config and generated one task | `simulationStore.ts`, `proceduralGenerator.ts` | Track user config overrides separately and restore Small Demo to six tasks | `debug-robustness.test.tsx`, browser smoke |
| SIM-P1-010 | Queue pre-point reservation acted like a task-long station lock | P1 | fixed | Generate 6 tasks with two stations and one pre-point per station | A queue pre-point is a physical checkpoint; followers wait upstream after it is occupied/reserved | Pre-point capacity blocked all later dispatch until service/task completion | `queuePointLifecycle.ts`, `queuePoints.ts`, `simulationEngine.ts` | Added `HOLD_UPSTREAM` dispatch semantics and release after checkpoint visit | `queuePointLifecycle.test.ts`, live smoke |
...
| TRAF-P0-005 | Robots could attempt or accept entry into an occupied physical cell | P0 | fixed | Diagnostics showed `robot_001` and `robot_002` both owning `20:26` while the station cell was occupied | A robot must be granted ownership of a cell/envelope before it moves toward it | The simulator moved optimistically with `advanceRobotOnPath` and repaired collisions after the fact | `trafficMoveGate.ts`, `simulationEngine.ts`, `diagnosticsExport.ts` | Added deterministic pre-move traffic gate, denied-move diagnostics, and duplicate current/target-cell invariants | `trafficMoveGate.test.ts`, `traffic-control.spec.ts` |
...
| PATH-P1-005 | Direction was stored as cell-local state rather than canonical edges | P1 | fixed | Compare arrows, graph builder, and path planner | UI arrows and runtime movement use the same directed neighbor links | Direction editing could be interpreted differently by UI and routing | `directionLinks.ts`, `graphBuilder.ts`, `DirectionArrowLayer.tsx`, `layoutStore.ts` | Added persisted `DirectedNeighborLink` model with import derivation from old cell directions | `semantic-correction.test.ts`, `generator.test.ts` |
