...
export function validateSimulationReadiness(layout: WarehouseLayout): SimulationReadinessResult {
  ...
  if (normalized.stations.some((station) => stationQueueCells(normalized, station).length === 0)) categories.stations.push("Stations need at least one linked queue lane.");
