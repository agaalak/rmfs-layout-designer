...
export function validateSimulationReadiness(layout: WarehouseLayout): SimulationReadinessResult {
  ...
  if (normalized.stations.some((station) => station.maxQueueLength < 1 && station.queueCells.length === 0)) categories.stations.push("Stations need queue cells or positive queue capacity.");
